...
function generateMemberLedger(company_id,member_id,cb) {
    ...
    workbook.xlsx.readFile(path.resolve(__dirname, template_path))
        .then(function() {
            ...
            let filename = token+"-m.xlsx";
            let worksheet = workbook.getWorksheet(1);
            let rowCount = 5;
            ShareTransfer.find({where:{company_id:company_id,approved:true,transferer_type:'shareholder',and:[{or:[{transferer_id:member_id},{transferee_id:member_id}]}]},include: ['transferer', 'transferee', 'sharetype']})
                .then((res)=>{
...
module.exports = generateMemberLedger;